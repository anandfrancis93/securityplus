  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-8 max-w-7xl">
        ...
        {/* Options Grid - MD3 Card Pattern with Elevation */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 pb-12">
          {options.map((option, index) => (
            <button
              ...
              className={`group relative bg-gradient-to-br ${option.gradient}
                       backdrop-blur-sm rounded-[28px] p-10 md:p-12 border-2 ${option.borderColor}
                       cursor-pointer transition-all duration-500 ease-out
                       animate-in fade-in slide-in-from-bottom-8 duration-700
                       hover:scale-[1.02] active:scale-[0.98]
                       hover:shadow-2xl hover:${option.glowColor}
                       focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:ring-offset-2 focus:ring-offset-slate-950`}
              style={{
                animationDelay: `${index * 100}ms`,
                background: hoveredCard === option.id
            ...
            >
              ...
              {hoveredCard === option.id && (
                <div className={`absolute inset-0 rounded-[28px] blur-xl ${option.gradient} opacity-50 -z-10 transition-opacity duration-500`} />
              )}

              {/* Icon with MD3 Scale Animation */}
              <div className={`flex justify-center items-center mb-6 transition-transform duration-500 ease-out ${option.iconColor}
                           ${hoveredCard === option.id ? 'scale-110 rotate-3' : 'scale-100 rotate-0'}`}>
                {option.icon}
              </div>

              {/* Option Name - MD3 Headline Typography */}
              <h2 className="text-3xl md:text-4xl font-bold mb-4 text-white tracking-tight relative z-10 text-center">
                {option.name}
              </h2>

              {/* Description - MD3 Body Typography */}
              <p className="text-slate-400 text-base md:text-lg leading-relaxed relative z-10 text-center">
                {option.description}
              </p>

              {/* Active Indicator - MD3 Pattern */}
              <div className={`absolute bottom-5 right-5 transition-all duration-300
                            ${hoveredCard === option.id ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-2'}`}>
                <svg
